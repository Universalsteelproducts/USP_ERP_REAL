...
-->
    ...
    <#if userLogin??>
        ...
        <#if parameters.componentName?? && requestAttributes._CURRENT_VIEW_?? && helpTopic??>
            ...
            <li>
                <a class="help-link <#if pageAvail?has_content> alert</#if>" href="javascript:lookup_popup1('showHelp?helpTopic=${helpTopic}&amp;portalPageId=${parameters.portalPageId!}','help' ,500,500);" title="${uiLabelMap.CommonHelp}"></a>
            </li>
        </#if>
        <li>
    ...
    <#else>
        <li>
            <a href="<@ofbizUrl>${checkLoginUrl}</@ofbizUrl>">
                ${uiLabelMap.CommonLogin}
            </a>
        </li>
    </#if>
        <li <#if companyListSize?default(0) &lt;= 1>class="language"</#if>>
            <a href="<@ofbizUrl>ListLocales</@ofbizUrl>">
    ...
    <#if userLogin?exists>
        <#if userLogin.partyId?exists>
            <li class="user">
                <a href="/partymgr/control/viewprofile?partyId=${userLogin.partyId}${externalKeyParam!}">
                    ${userName}
                </a>&nbsp;&nbsp;&nbsp;&nbsp;
            </li>
